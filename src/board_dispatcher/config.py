...
class BoardDispatcherConfig(BaseModel):
    ...
    # Workspace
    workspace_root: str = "/tmp/board-dispatcher-workspaces"
    ...
    @classmethod
    def load(cls, path: str | Path = "config.yaml") -> BoardDispatcherConfig:
        ...
        bd = raw.get("board_dispatcher", raw)
        ...
        config = cls(
            ...
            jira_label=bd.get("jira", {}).get("label", "claudio"),
            ...
            workspace_root=_expand(
                bd.get("workspace", {}).get("root", "/tmp/board-dispatcher-workspaces")
            ),
