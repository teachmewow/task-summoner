...
class TicketState(str, Enum):
    """Full lifecycle states for the board-dispatcher."""
...
def state_from_labels(labels: list[str]) -> TicketState | None:
    """Extract the most advanced claudio:<state> from Jira labels.

    Returns None if no claudio state labels are found.
    """
    found: list[TicketState] = []
    for label in labels:
        if label.startswith("claudio:"):
            state_str = label.removeprefix("claudio:").upper()
            try:
