...
Each comment posted by task-summoner includes a machine-readable tag:
  [ts:{ticket_key}:{state}:{short_id}]
...
_TS_TAG_PATTERN = re.compile(r"\[ts:([A-Z]+-\d+):([a-z_]+):([a-z0-9]+)\]")


class MessageTag(BaseModel):
    """A unique tag embedded in task-summoner comments."""
    ...
    @property
    def tag(self) -> str:
        return f"[ts:{self.ticket_key}:{self.state}:{self.short_id}]"
...
def find_ts_comment(comments: list[dict], tag_str: str) -> int | None:
...
def is_ts_comment(comment: dict) -> bool:
    """Check if a comment was posted by task-summoner (contains a bd tag)."""
    body = str(comment.get("body", ""))
    return bool(_TS_TAG_PATTERN.search(body))


def find_latest_ts_tag(comments: list[dict], ticket_key: str, state: str) -> str | None:
    ...
    [ts:LLMOPS-1146:implementing:abc12345]. Returns the full tag string
    or None if not found. Used for state recovery when metadata is lost.
    """
    pattern = re.compile(rf"\[ts:{re.escape(ticket_key)}:{re.escape(state)}:[a-z0-9]+\]")
...
def get_replies_after(comments: list[dict], tag_str: str) -> list[dict]:
    ...
    """
    idx = find_ts_comment(comments, tag_str)
    if idx is None:
        return []
