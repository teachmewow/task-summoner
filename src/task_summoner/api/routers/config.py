"""Config endpoints — web setup page + save/test/status.

Consolidates what used to live in `api/setup.py`. The setup HTML is served
from `/setup`; the JSON endpoints under `/api/config/*` handle state.
"""
...
_STATIC_DIR = Path(__file__).resolve().parent.parent.parent / "dashboard_ui" / "static"
