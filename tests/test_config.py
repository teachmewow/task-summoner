...
class TestConfigLoading:
    def test_load_from_yaml(self, config_file: Path):
        config = BoardDispatcherConfig.load(config_file)
    ...
    def test_missing_file_returns_defaults(self, tmp_path: Path):
        config = BoardDispatcherConfig.load(tmp_path / "nonexistent.yaml")
        assert config.poll_interval_sec == 15
        assert config.jira_label == "claudio"


class TestConfigValidation:
    def test_missing_api_key(self, config: BoardDispatcherConfig):
    ...
    def test_missing_repo_path(self):
        config = BoardDispatcherConfig(repos={"fake": "/nonexistent"}, default_repo="fake")
        errors = config.check_config()
        assert any("does not exist" in e for e in errors)

    def test_no_repos(self):
        config = BoardDispatcherConfig(repos={})
        errors = config.check_config()
...
class TestResolveRepo:
    def test_resolve_from_label(self):
        config = BoardDispatcherConfig(repos={"api": "/tmp/api", "ui": "/tmp/ui"}, default_repo="api")
        name, path = config.resolve_repo(["claudio", "repo:ui"])
        assert name == "ui"

    def test_resolve_fallback(self):
        config = BoardDispatcherConfig(repos={"api": "/tmp/api"}, default_repo="api")
        name, _ = config.resolve_repo(["claudio"])
        assert name == "api"
    ...
    def test_no_default_raises(self):
        config = BoardDispatcherConfig(repos={"api": "/tmp/api"}, default_repo="")
        with pytest.raises(ValueError):
            config.resolve_repo(["claudio"])
